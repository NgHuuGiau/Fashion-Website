...
- Name: products.spec.ts >> Product Browsing >> should display home page with products
- Location: tests\products.spec.ts:10:7
...
Locator: locator('h1, .hero, .product-grid, .products')
Expected: visible
Error: strict mode violation: locator('h1, .hero, .product-grid, .products') resolved to 2 elements:
    1) <h1>Gọn mà có gu, mặc mỗi ngày.</h1> aka getByRole('heading', { name: 'Gọn mà có gu, mặc mỗi ngày.' })
    2) <div class="product-grid product-grid-editorial">…</div> aka locator('div').filter({ hasText: 'NỔI BẬT Quần Quần jeans Baggy' }).first()

Call log:
  - Expect "toBeVisible" locator('h1, .hero, .product-grid, .products') with timeout 10000ms
  - waiting for locator('h1, .hero, .product-grid, .products')
