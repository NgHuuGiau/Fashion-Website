(function () {
  ...
  document.addEventListener("click", (e) => {
    const thumb = e.target.closest("[data-detail-image]");
    if (!thumb) return;
    const url = thumb.dataset.detailImage;
    if (!url) return;
    const main = document.getElementById("detail-main-image");
    if (!main) return;
    main.style.opacity = "0";
    setTimeout(() => { main.src = url; main.alt = thumb.dataset.detailAlt || main.alt; main.style.opacity = "1"; }, 120);
    document.querySelectorAll("[data-detail-image]").forEach((t) => { t.classList.remove("active"); });
  ...
  document.addEventListener("click", (e) => {
    ...
    let overlay = document.getElementById("gallery-zoom");
    if (!overlay) {
  ...
  document.addEventListener("click", (e) => {
    const overlay = document.getElementById("gallery-zoom");
  ...
  document.addEventListener("click", (e) => {
    ...
    const input = wrap ? wrap.querySelector("[data-qty-input]") : null;
    if (!input) return;
    const min = parseInt(input.min || "1", 10);
    const max = parseInt(input.max || "9999", 10);
    const cur = parseInt(input.value || String(min), 10);
    const next = Math.max(min, Math.min(max, (isNaN(cur) ? min : cur) + step));
  ...
  if (launcher && chatbox && closeBtn && chatForm && chatInput && chatMessages && backdrop) {
    ...
    const getFallbackReply = (message) => {
      ...
      if (/size|kích[\s\S]*c[oồ]|vừa|m[\s\S]*c|1m|m[\s\S]*t|cao.*cân|chi[eê]u cao/i.test(text)) {
        const match = text.match(/(\d{1,2})\s*m\s*(\d{1,3})(?:\s+(\d{1,3})\s*kg)?/i);
        if (match) {
          const h = parseInt(match[1], 10) * 100 + parseInt(match[2], 10);
          const w = match[3] ? parseInt(match[3], 10) : 0;
    ...
    const sendMessage = (text) => {
      ...
      setTimeout(() => {
        fetchReply(cleanText).then(result => {
          ...
          if (loadingMsg && loadingMsg.parentNode) loadingMsg.remove();
          appendMessage("bot", getFallbackReply(cleanText));
          chatInput.disabled = false;
        });
      }, 180);
    };
  ...
  document.addEventListener("click", (e) => {
    const btn = e.target.closest("[data-gallery-dir]");
    if (!btn) return;
    const dir = parseInt(btn.dataset.galleryDir, 10);
    const thumbs = document.querySelectorAll("[data-detail-image]");
    if (!thumbs.length) return;
    const arr = Array.from(thumbs);
    const idx = arr.findIndex((x) => { return x.classList.contains("active"); });
    const next = dir === 1
  ...
  document.addEventListener("click", (e) => {
    const overlay = e.target.closest(".size-guide-overlay");
  ...
  (function () {
    ...
    function showTab (targetId) {
      ...
      const targetTab = document.querySelector(`.admin-tab[data-target="${targetId}"]`);
  ...
  (function () {
    ...
    const table = document.querySelector(".admin-product-table tbody");
    if (!table) return;
  ...
  (function () {
    const headerCheck = document.getElementById("admin-select-all-header");
    const footerCheck = document.getElementById("admin-select-all");
    if (!headerCheck) return;
    const checks = document.querySelectorAll(".admin-product-check");
    const bulkIds = document.getElementById("admin-bulk-ids");
    const bulkIdsShow = document.getElementById("admin-bulk-ids-show");
    const hideBtn = document.getElementById("admin-bulk-hide-btn");
    const showBtn = document.getElementById("admin-bulk-show-btn");
    function updateBulk () {
      const selected = [];
    ...
    if (footerCheck) {
      footerCheck.addEventListener("change", () => {
        checks.forEach((c) => { c.checked = footerCheck.checked; });
  ...
  (function () {
    ...
    const mainForm = document.getElementById("detail-buy-form");
    if (!sticky || !mainForm) return;
    const variantId = document.getElementById("variant-id-input");

    let shown = false;
    function maybeShow () {
      const nearBottom = window.innerHeight + window.scrollY >= document.body.scrollHeight - 480;
      const shouldShow = window.innerWidth <= 900 && window.scrollY > 420 && !nearBottom;
      if (shouldShow && !shown) { sticky.classList.add("is-visible"); sticky.setAttribute("aria-hidden", "false"); shown = true; } else if (!shouldShow && shown) { sticky.classList.remove("is-visible"); sticky.setAttribute("aria-hidden", "true"); shown = false; }
    }
    ...
    function syncToSticky () {
      if (variantId && variantId.value) {
        const label = variantId.getAttribute("data-variant-label");
        const text = sticky.querySelector(".sticky-buy-info span");
        if (label && text) text.textContent = label;
      }
    }

    sticky.querySelectorAll("[data-detail-buy-submit]").forEach((btn) => {
      btn.addEventListener("click", () => {
        const next = btn.dataset.stickyNext || "";
        mainForm.querySelectorAll('input[name="next"]').forEach((n) => { n.remove(); });
        if (next) {
        ...
        if (variantId && variantId.value) {
          const existing = mainForm.querySelector('input[name="variant_id"]');
  ...
  (function () {
    const el = document.querySelector("[data-viewers-count]");
    if (!el) return;
    let base = parseInt(el.textContent, 10) || 12;
    el.textContent = base;
    function drift () {
      let step;
  ...
  (function () {
    const timer = document.querySelector("[data-flash-sale]");
    if (!timer) return;
    const hEl = timer.querySelector("[data-flash-h]");
    const mEl = timer.querySelector("[data-flash-m]");
    const sEl = timer.querySelector("[data-flash-s]");
  ...
  document.querySelectorAll(".detail-accordion, .size-advisor").forEach((acc) => {
